...
class FollowBehindNode(Node):
    def __init__(self):
        super().__init__('follow_behind_node')
        ...
        self.get_logger().info('FollowBehindNode started.')

    def april_cb(self, msg: PoseStamped):
        """Maintain 1 m ahead of tag (so move forward when tag gets too close)."""
        self.no_tag = False
        self.last_seen_time = self.get_clock().now()
        ...
        # Inverted logic: move away if tag (person) too close
        ...
        # Steering toward tag center (same as lead_ahead)
    ...
    def no_tag_cb(self, msg: Bool):
        if msg.data and not self.no_tag:
            self.get_logger().warn('AprilTag lost slowing to stop.')
        self.no_tag = msg.data
